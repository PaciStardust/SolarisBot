...
{
    internal sealed class DatabaseContext : DbContext //todo: [UPCOMING] bdays, temp channels, gif-ify, stats, steal nickname, docker support, light control, hydro reminders, user analysis, uptime, discord sink, embed gen, module editor, google/wiki search
    {
        private readonly ILogger<DatabaseContext> _logger;
        private static bool _hasMigrated = false;

        public DatabaseContext(DbContextOptions<DatabaseContext> options, ILogger<DatabaseContext> logger) : base(options) //todo: [REFACTOR] Should this be used with service?
        {
            _logger = logger;
            TryMigrate();
        }
